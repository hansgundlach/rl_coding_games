#!/bin/bash
#SBATCH --job-name=spiral_prisoners_dilemma
#SBATCH --output=logs/spiral_prisoners_dilemma_job_%j_%x/slurm-spiral-pd-%j.out
#SBATCH --error=logs/spiral_prisoners_dilemma_job_%j_%x/slurm-spiral-pd-%j.err
...
# MIT Supercloud SLURM script for SPIRAL Prisoner's Dilemma Training
# Single V100 GPU training with logs output
#
# Usage: sbatch submit_spiral_prisoners_dilemma.sh [config_overrides...]
# Examples:
#   sbatch submit_spiral_prisoners_dilemma.sh --training.num_steps=50 --game.num_rounds=50
#   sbatch submit_spiral_prisoners_dilemma.sh --training.games_per_step_v100=3 --evaluation.eval_interval_steps=10
...
echo "🎯 Starting SPIRAL Prisoner's Dilemma Training on Supercloud V100"
echo "================================================================="
...
JOB_LOG_DIR="logs/spiral_prisoners_dilemma_job_${SLURM_JOB_ID}_${TIMESTAMP}"
mkdir -p "$JOB_LOG_DIR"
...
echo "🏋️ Starting SPIRAL prisoner's dilemma training at $(date)..."
PYTHON_CMD="python spiral_prisoners_dilemma.py --config configs/spiral_prisoners_dilemma.yaml"
if [ ! -z "$CONFIG_OVERRIDES" ]; then
...
# Copy prisoner's dilemma specific results
...
SPIRAL Prisoner's Dilemma Training Job Summary
==============================================
...
Game Description:
- Iterated Prisoner's Dilemma with strategy code submission
