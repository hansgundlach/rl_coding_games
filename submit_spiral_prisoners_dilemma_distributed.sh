#!/bin/bash
#SBATCH --job-name=spiral_pd_distributed
#SBATCH --output=logs/spiral_pd_distributed_job_%j_%x/slurm-spiral-pd-dist-%j.out
#SBATCH --error=logs/spiral_pd_distributed_job_%j_%x/slurm-spiral-pd-dist-%j.err
...
# MIT Supercloud SLURM script for Distributed SPIRAL Prisoner's Dilemma Training
# Dual V100 GPU distributed training for maximum speed
#
# Usage: sbatch submit_spiral_prisoners_dilemma_distributed.sh [config_overrides...]
# Examples:
#   sbatch submit_spiral_prisoners_dilemma_distributed.sh --training.num_steps=50 --game.num_rounds=50
#   sbatch submit_spiral_prisoners_dilemma_distributed.sh --training.games_per_step_v100=3 --evaluation.eval_interval_steps=10
...
echo "🚀 Starting Distributed SPIRAL Prisoner's Dilemma Training on Supercloud V100s"
echo "=============================================================================="
...
JOB_LOG_DIR="logs/spiral_pd_distributed_job_${SLURM_JOB_ID}_${TIMESTAMP}"
mkdir -p "$JOB_LOG_DIR"
echo "📁 Created job log directory: $JOB_LOG_DIR"

# Export log directory for Python script to use
export SPIRAL_PD_LOG_DIR="$JOB_LOG_DIR"

# Set up distributed training environment variables
export MASTER_ADDR=localhost
export MASTER_PORT=12356  # Different port from GRPO to avoid conflicts
export WORLD_SIZE=2
...
echo "🏋️ Starting distributed SPIRAL prisoner's dilemma training at $(date)..."
...
TORCHRUN_CMD="$TORCHRUN_CMD spiral_prisoners_dilemma_distributed.py"
TORCHRUN_CMD="$TORCHRUN_CMD --config configs/spiral_prisoners_dilemma.yaml"
...
cleanup() {
    ...
    pkill -f "spiral_prisoners_dilemma_distributed.py" || true
    sleep 2
}
trap cleanup EXIT
...
echo ""
echo "🏁 Distributed SPIRAL training completed at $(date)"
...
cp configs/spiral_prisoners_dilemma.yaml "$JOB_LOG_DIR/" 2>/dev/null || echo "Config file not found"
...
# Copy SPIRAL prisoner's dilemma specific results
if [ -d "eval_results/spiral_prisoners_dilemma" ]; then
    echo "🧪 Copying SPIRAL prisoner's dilemma evaluation results..."
    cp -r eval_results/spiral_prisoners_dilemma "$JOB_LOG_DIR/"
fi
...
Distributed SPIRAL Prisoner's Dilemma Training Job Summary
==========================================================
...
Game Description:
- Distributed Iterated Prisoner's Dilemma with strategy code submission
- Players write Python functions to decide cooperate/defect
- Game generation distributed across 2 V100 GPUs
- Synchronized Role-conditioned Advantage Estimation (RAE)
- Rewards based on tournament wins/losses
...
echo "🎉 Distributed SPIRAL SLURM job completed!"
