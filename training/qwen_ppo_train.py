...
from training.lora_utils import setup_lora_model, save_lora_checkpoint, prepare_model_for_training
...
class QwenPPOExperience:
    ...
    def __init__(self, state, action, reward, next_state, done, log_prob, prompt, response):
        self.state = state
        self.action = action
        self.reward = reward
...
class QwenConnectXPPOTrainer:
    ...
    def compute_rewards_to_go(self, experiences: List[QwenPPOExperience]) -> List[float]:
    ...
    def update_qwen_model(self, batch_experiences: List[QwenPPOExperience]) -> Dict[str, float]:
        ...
        for i in range(0, len(batch_experiences), batch_size):
            ...
            full_texts = [f"{prompt}{response}" for prompt, response in zip(mini_batch_prompts, mini_batch_responses)]
    ...
    def train(self):
        ...
        for epoch in range(training_config['num_epochs']):
            ...
            if (epoch + 1) % self.config['checkpoints'].get('save_interval', 5) == 0:
                ...
                checkpoint_path = os.path.join(checkpoint_dir, f"qwen_ppo_epoch_{epoch + 1}")
