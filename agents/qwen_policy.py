...
Your move (column number):"""
    ...
    def _generate_qwen_action(self, board: np.ndarray, configuration: Dict[str, Any], valid_actions: list) -> int:
        ...
        try:
            ...
            if hasattr(self.model, 'device') and self.model.device != torch.device("cpu"):
                try:
            ...
            try:
                ...
                torch.cuda.empty_cache()
                print("Warning: GPU OOM during generation, retrying with reduced settings")
                with torch.no_grad():
                    outputs = self.model.generate(
                        **inputs,
                        max_new_tokens=5,
