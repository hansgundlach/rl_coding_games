#!/bin/bash
#SBATCH --job-name=grpo_code_game_icl
#SBATCH --output=logs/job_%j_%x/slurm-grpo-code-game-icl-%j.out
#SBATCH --error=logs/job_%j_%x/slurm-grpo-code-game-icl-%j.err
...
# MIT Supercloud SLURM script for GRPO Code Game with ICL Memory Training
# Single V100 GPU training with logs output
#
# Usage: sbatch submit_grpo_code_game_icl.sh [config_overrides...]
# Examples:
#   sbatch submit_grpo_code_game_icl.sh --training.num_steps=50 --icl.refresh_every=10
#   sbatch submit_grpo_code_game_icl.sh --evaluation.num_questions=10 --training.games_per_step=4
...
echo "🚀 Starting GRPO Code Game with ICL Memory Training on Supercloud V100"
echo "=================================================================="
...
echo "🏋️ Starting GRPO Code Game with ICL Memory training at $(date)..."
PYTHON_CMD="python grpo_code_game_icl.py --config configs/grpo_code_game_icl.yaml"
if [ ! -z "$CONFIG_OVERRIDES" ]; then
...
GRPO Code Game with ICL Memory Training Job Summary
=================================================
...
- Guesser (Player 2): Frozen weights + ICL memory accumulation
- Memory snapshots for training stability (80/20 mixing)
