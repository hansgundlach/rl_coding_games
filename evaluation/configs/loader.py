...
def load_env_overrides() -> Dict[str, Any]:
    ...
    env_mappings = {
        ...
        'MBPP_EVAL_AT_START': ('eval_at_start', lambda x: x.lower() in ['true', '1', 'yes']),
        'MBPP_EVAL_AT_END': ('eval_at_end', lambda x: x.lower() in ['true', '1', 'yes']),
        'MBPP_EVAL_INTERVAL_STEPS': ('eval_interval_steps', lambda x: int(x) if x.lower() != 'null' else None),
        'MBPP_DATASET_PATH': ('dataset_path', str),
        'MBPP_USE_SANITIZED': ('use_sanitized', lambda x: x.lower() in ['true', '1', 'yes']),
        'MBPP_TEST_SPLIT': ('test_split', str),
        'MBPP_TIMEOUT_SECONDS': ('timeout_seconds', int),
        'MBPP_MAX_MEMORY_MB': ('max_memory_mb', int),
        'MBPP_SAFE_EXECUTION': ('safe_execution', lambda x: x.lower() in ['true', '1', 'yes']),
        ...
        'MBPP_SAVE_RESULTS': ('save_results', lambda x: x.lower() in ['true', '1', 'yes']),
    ...
    for env_var, (config_key, converter) in env_mappings.items():
        ...
        if env_value is not None:
            try:
                ...
                logger.info(f"Override from {env_var}: {config_key}={overrides[config_key]}")
