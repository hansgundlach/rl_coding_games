    ...
    def start_server(self) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "--max-model-len", str(self.config.server["max_model_len"]),
                "--tensor-parallel-size", str(self.config.server["tensor_parallel_size"]),
                "--max-num-batched-tokens", str(self.config.server["max_num_batched_tokens"]),
                "--max-num-seqs", str(self.config.server["max_num_seqs"]),
                "--swap-space", str(self.config.server["swap_space"]),
                "--dtype", self.config.server["dtype"],
            ]
            
            # Add offline mode flags
            if self.config.server.get("offline_mode", True):
                cmd.extend(["--disable-log-requests"]) if self.config.server.get("disable_log_requests") else None
...
class VLLMClient:
    ...
    def generate_completions(self, prompts: Union[str, List[str]], mode: str = "grpo_completions") -> List[str]:
        """Generate completions using vLLM server"""
        if isinstance(prompts, str):
            prompts = [prompts]
            
        generation_config = self.config.generation.get(mode, self.config.generation["grpo_completions"])
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"vLLM request failed (attempt {attempt+1}): {response.status_code}")
...
class VLLMIntegration:
    ...
    def __init__(self, config: Dict[str, Any], model_path: str, offline_mode: bool = True):
        ...
        if self.vllm_config.enabled:
            self.server_manager = VLLMServerManager(self.vllm_config, model_path, offline_mode)
...
def initialize_vllm_integration(config: Dict[str, Any], model_path: str, offline_mode: bool = True) -> VLLMIntegration:
